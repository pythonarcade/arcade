...
def test_perspective_projector_activate(window: Window):
    ...
    # When
    with camera_default.activate() as cam:
        # Initially
        assert window.current_camera is cam is camera_default
        assert window.ctx.view_matrix == view_matrix
        assert window.ctx.projection_matrix == proj_matrix
...
def test_perspective_projector_map_coordinates_move(window: Window):
    ...
    # Then
    assert camera_default.map_coordinate(mouse_pos_a) == pytest.approx((0.0, 0.0, 0.0))
    assert camera_default.map_coordinate(mouse_pos_b) == pytest.approx((-300.0, -200.0, 0.0))
    ...
    # Then
    assert camera_default.map_coordinate(mouse_pos_a) == pytest.approx((100.0, 100.0, 0.0))
    assert camera_default.map_coordinate(mouse_pos_b) == pytest.approx((-200.0, -100.0, 0.0))


def test_perspective_projector_map_coordinates_rotate(window: Window):
    # Given
    camera_default = camera.PerspectiveProjector()
    default_view = camera_default.view
    ...
    # Then
    assert camera_default.map_coordinate(mouse_pos_a) == pytest.approx((0.0, 0.0, 0.0))
    assert camera_default.map_coordinate(mouse_pos_b) == pytest.approx((-200.0, 300.0, 0.0))
    ...
    # Then
    assert camera_default.map_coordinate(mouse_pos_a) == pytest.approx((100.0, 100.0, 0.0))
    assert camera_default.map_coordinate(mouse_pos_b) == pytest.approx((-253.553390, 170.710678, 0.0))


def test_perspective_projector_map_coordinates_zoom(window: Window):
    # Given
    camera_default = camera.PerspectiveProjector()
    default_view = camera_default.view
    ...
    # Then
    assert camera_default.map_coordinate(mouse_pos_a) == pytest.approx((window.width*0.75, window.height*0.75, 0.0))
    assert camera_default.map_coordinate(mouse_pos_b) == pytest.approx((250.0, 200.0, 0.0))
