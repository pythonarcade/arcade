...
def test_rotation_mirror():
    ...
    assert "torch1" in sprite.frames[0].texture.origin
    assert sprite.frames[1].duration == 500
    assert "torch2" in sprite.frames[1].texture.origin

    sprite.update_animation(0.501)
    assert "torch2" in sprite.texture.origin
    sprite.update_animation(0.501)
    assert "torch1" in sprite.texture.origin
