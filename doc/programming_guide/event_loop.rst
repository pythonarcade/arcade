...
:py:class:`arcade.Window` with the ``draw_rate`` argument.

Setting the draw rate to a value above a screen's refresh rate can cause tearing
unless you set the ``vsync`` argument to true. We recommend keeping your
``draw_rate`` around the screen's refresh rate. However, most users should just
use the default values. Having a draw rate higher than the update rate is not
possible and will raise an error.
...
called with the ``fixed_rate`` argument when initializing your :py:class:`arcade.Window`.
