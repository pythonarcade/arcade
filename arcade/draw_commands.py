...
def draw_circle_filled(
    ...
    draw_ellipse_filled(
        center_x, center_y, radius * 2, radius * 2, color, 0, num_segments
    )
...
def draw_circle_outline(
    ...
    draw_ellipse_outline(
        center_x, center_y, radius * 2, radius * 2, color, border_width, num_segments
    )
...
def _generic_draw_line_strip(
    point_list: PointList, color: Color, mode: int = gl.GL_LINE_STRIP
):
    """
...
def draw_triangle_filled(
    x1: float, y1: float, x2: float, y2: float, x3: float, y3: float, color: Color
):
    """
...
def draw_xywh_rectangle_outline(
    bottom_left_x: float,
    bottom_left_y: float,
    width: float,
    height: float,
...
def draw_lrtb_rectangle_filled(
    left: float, right: float, top: float, bottom: float, color: Color
):
    """
    Draw a rectangle by specifying left, right, top, and bottom edges.
    ...
    if left > right:
        raise AttributeError(
            "Left coordinate {} must be less than or equal "
            "to the right coordinate {}".format(left, right)
        )

    if bottom > top:
        raise AttributeError(
            "Bottom coordinate {} must be less than or equal "
...
def draw_xywh_rectangle_filled(
    bottom_left_x: float,
    bottom_left_y: float,
    width: float,
    height: float,
