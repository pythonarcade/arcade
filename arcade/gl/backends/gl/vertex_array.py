...
from arcade.gl.new import Context, Geometry, Program, VertexArray
...
class GLVertexArray(VertexArray):
    ...
    def __init__(
        ...
        if self._ctx.gc_mode == "auto":
            weakref.finalize(self, GLVertexArray.delete_glo, self._ctx, glo)

        self._ctx.stats.incr("vertex_array")
    ...
    @staticmethod
    def delete_glo(ctx: GLContext, glo: gl.GLuint) -> None:
        """
    ...
    def _build(
        self, program: GLProgram, content: Sequence[BufferDescription], index_buffer: GLBuffer | None
    ) -> None:
        """
    ...
    def render_indirect(self, buffer: GLBuffer, mode: GLuintLike, count, first, stride) -> None:
        """
        ...
        gl.glBindVertexArray(self.glo)
        gl.glBindBuffer(gl.GL_DRAW_INDIRECT_BUFFER, buffer.glo)
        if self._ibo:
...
class GLGeometry(Geometry):
    ...
    def __init__(
        ...
        self._vao_cache: dict[str, GLVertexArray] = {}
        ...
        weakref.finalize(self, GLGeometry._release, self._ctx)
    ...
    @property
    def index_buffer(self) -> GLBuffer | None:
    ...
    def instance(self, program: GLProgram) -> GLVertexArray:
        """
    ...
    def transform(
        ...
        *,
        ...
        if program._varyings_capture_mode == "interleaved":
            if not isinstance(buffer, GLBuffer):
                raise ValueError(
                    (
    ...
    def _generate_vao(self, program: GLProgram) -> GLVertexArray:
        """
