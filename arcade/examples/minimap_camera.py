...
class MyGame(arcade.Window):
    ...
    def __init__(self, width, height, title):
        ...
        minimap_projection = arcade.LRBT(
            -MAP_PROJECTION_WIDTH / 2,
            MAP_PROJECTION_WIDTH / 2,
            -MAP_PROJECTION_HEIGHT / 2,
            MAP_PROJECTION_HEIGHT / 2,
        )
        ...
        # texts
        text = (
            'Press "A" to select minimap camera. Press "B" to select main camera. Press "W" and "S" to increase '
            'or decrease zoom level on the selected camera.\nPress "I" and "K" to enlarge or reduce minimap.'
        )
        self.instructions = arcade.Text(
            text, 10, 25, arcade.color.BLACK_BEAN, 10, multiline=True, width=DEFAULT_SCREEN_WIDTH
        )
