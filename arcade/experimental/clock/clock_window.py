...
class Window(pyglet.window.Window):
    ...
    def __init__(
        ...
        # Arcade provides two clocks which are ticked every update and fixed update.
        ...
        self._fixed_update_rate = fixed_update_rate
        self.set_update_rate(update_rate)

        # If updating physics takes longer than the time step it represents the engine can start to take
        # longer and longer every frame. This caps the number of fixed updates that can happen.
    ...
    # I suspect it would be a bad idea. We could also use inheritance to make
    # a stripped down clock similar to the basic camera.
    @property
    def update_clock(self):
        return self._update_clock
    ...
    def on_update(self, delta_time: float):
        """
        Perform game logic which does not need regular interval updates.
        For stability do not put physics updates such as collisions here.
