...
## 12. CI (GitHub Actions, Linux + xvfb) failures on PR #2865 — two real bugs, one non-blocking pre-existing race
...
**Bug 2 — the new stress test polluted global window state for later tests** (confirmed, fixed): `tests/integration/test_ubo_stress.py` runs early in collection order (3rd file in the CI job, right after `tests/integration/examples/test_examples.py`). It created and closed 100 windows via `arcade.set_window(window)` without ever restoring the *original* global window afterward, leaving `arcade.get_window()` pointing at a closed window for the rest of the session. This is a very plausible explanation for failures that only appeared *after* it in collection order and were never seen on the unmodified `development` branch (which has a consistently green CI history — checked via `gh run list`): `test_gl_geometry.py`'s `geo.ctx == ctx` assertions (comparing two different `ArcadeContext` instances), `test_gl_gc.py`'s off-by-one object counts, `test_gl_framebuffer.py::test_clear_viewport`, and `test_exp_restricted_input.py`'s `GLException`. **Fix**: both stress test functions now save `arcade.get_window()` (via `arcade.window_exists()`) before running and restore it in a `finally` block; stress windows are also created with `visible=False` to reduce window-manager/X11 overhead. Verified locally by running `tests/integration/test_ubo_stress.py` together with `tests/unit/gl/` and `tests/unit/gui/test_exp_restricted_input.py` (previously reproduced the failures in combination) — all 93 pass together after the fix.

**Non-blocking, pre-existing, out of scope**: a `PytestUnhandledThreadExceptionWarning` (`AttributeError: 'Window' object has no attribute '_ctx'`) surfaces from `arcade/examples/threaded_loading.py`'s background loading thread calling `get_window().ctx` while a new `Window` is mid-construction. Root cause: `Window.__init__` calls `set_window(self)` *before* `self._ctx` is assigned (`arcade/application.py`) — a narrow, pre-existing race in Arcade itself, independent of pyglet version. It doesn't fail CI (no `filterwarnings = error` configured; it's counted under "warnings," not "failed"/"error" in the pytest summary), but `test_ubo_stress.py`'s rapid-fire window construction makes it far likelier to be *observed*, since it multiplies the number of narrow "no `_ctx` yet" windows created per run. Not fixed here — reordering `Window.__init__` was already investigated once in this migration (§4) and found unnecessary for its originally-predicted failure mode; doing it now for a different, pre-existing, non-blocking race would be new scope creep for this PR.
