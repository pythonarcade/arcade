...
**Still open**: whether this same `Context.active` corruption explains the reference-image test's 36% CI failure (as opposed to a genuine hardware-vs-software rasterizer difference) is not yet confirmed — the scenes' draw calls (`SpriteList.draw()`, `arcade.draw_circle_filled()`, etc.) appear to route through `get_window()` rather than `Context.active`, so they may be unaffected by this specific bug, but this needs an actual CI re-run to know for sure rather than further local guessing.
